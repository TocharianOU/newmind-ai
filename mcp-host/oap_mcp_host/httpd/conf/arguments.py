...
from oap_mcp_host.httpd.conf.project_context import get_project_config_path
...
class Arguments(BaseModel):
    """Command line arguments of oap_httpd."""
    ...
    def rewrite_default_path(self) -> Self:
        ...
        cwd = Path(self.working_dir) if self.working_dir else OAP_CONFIG_DIR
        if not self.httpd_config:
            self.httpd_config = cwd.joinpath("oap_httpd.json")
    ...
    def parse_args(cls, args: list[str] | None = None) -> Self:
        ...
        parser = ArgumentParser(prog="oap_httpd", exit_on_error=False)
