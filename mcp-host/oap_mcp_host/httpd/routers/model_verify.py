...
from oap_mcp_host.host.conf import HostConfig, LogConfig
from oap_mcp_host.host.conf.llm import LLMConfigTypes
from oap_mcp_host.host.errors import ThreadQueryError
from oap_mcp_host.host.host import AttackTraceMcpHost
from oap_mcp_host.host.tools.misc import TestTool
from oap_mcp_host.httpd.dependencies import get_app, get_attacktrace_user
from oap_mcp_host.httpd.routers.utils import EventStreamContextManager
...
def get_verify_subjects(llm_config: LLMConfigTypes) -> list[VERIFY_SUBJECTS]:
    """Get the verify subjects."""
    if llm_config.model_provider == "ollama":
        logger.info(
            "Ollama provider detected, only check connection and tools_in_prompt"
        )
