...
def define_env(env):
	...
	def templatesWidget(title: str, slug: str, toLoad: int = 3) -> str:
		# Disabled: No external API calls for offline docs
		return '<div class="newmindchat-templates-widget"><p>Template browsing is not available in offline mode.</p></div>'

	@env.macro
	def workflowDemo(workflow_json):
		# Only support local file:// URLs, no external API calls
		parsed_workflow_url = urllib.parse.urlparse(workflow_json)

		if parsed_workflow_url.scheme == "file":
			try:
				...
				return f"<div class='newmindchat-workflow-preview'><n8n-demo hidecanvaserrors='true' clicktointeract='true' frame='false' collapseformobile='false' workflow='{encoded_workflow_json}'></n8n-demo><a href='{template_url}' target='_blank'>View workflow file</a></div>"
			except (FileNotFoundError, json.JSONDecodeError, KeyError) as e:
				return f"<div class='newmindchat-workflow-preview'><p>Workflow preview unavailable (local file not found).</p></div>"
		else:
			# No external API calls allowed
			return "<div class='newmindchat-workflow-preview'><p>Workflow preview unavailable (external URLs disabled for offline mode).</p></div>"
