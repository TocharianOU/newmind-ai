...
const Account = () => {
  ...
  const handleEmbeddedLoginSuccess = async (token: string) => {
    try {
      console.log('🚀 Account page: Starting embedded login with token:', token.substring(0, 8) + '...')
      
      // Use the new oapLoginWithToken function to trigger the complete OAP login flow
      await oapLoginWithToken(token)
      
      console.log('✅ Account page: OAP login flow initiated successfully')
    } catch (error) {
      console.error('❌ Account page: Login integration error:', error)
    } finally {
          ...
          <div className="profile-container">
            ...
            <div className="login-section-button-group">
              <Button
                ...
                size="fit"
                padding="xxl"
                onClick={() => setShowEmbeddedLogin(true)}
              >
                {t("common.login")}
              </Button>
              <Button
            ...
            <div className="plan-section">
              ...
              <div className="plan-details">
                ...
                <div className="subscription-info">
                  ...
                  <span className="info-value">
                    {countFormat(Math.max(0, (oapUsage?.limit ?? 0) - (oapUsage?.total ?? 0))) + " Token"}
                  </span>
                </div>
                    ...
                    <div className="progress-row">
                      <div className="progress-container">
                        <div className="progress-bar" style={
                          {
                            width: `${Math.min(100, (oapUsage?.total ?? 0) / (oapUsage?.limit ?? 1) * 100)}%`,
                            background: (oapUsage?.total ?? 0) > 0
                              ? `linear-gradient(
                                  to right,
                                  var(--bg-pri-blue-strong) 0%,
                                  var(--bg-pri-blue-strong) ${(oapUsage?.model ?? 0) / (oapUsage?.total ?? 1)  * 100}%,
                                  var(--bg-pri-blue-mediumweak) ${(oapUsage?.model ?? 0) / (oapUsage?.total ?? 1)  * 100}%,
                                  var(--bg-pri-blue-mediumweak) 100%
                                )`
                              : 'var(--bg-pri-blue-mediumweak)'
                          }
                        }></div>
                      </div>
                ...
                {(oapLevel === "BASE" || isOAPUsageLimit) &&
                  <div className={`warning ${isOAPUsageLimit ? "on-limit" : ""}`}>
                    {isOAPUsageLimit ?
