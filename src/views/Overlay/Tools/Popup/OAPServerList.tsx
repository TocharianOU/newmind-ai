...
const OAPServerList = ({
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // Apply MCP servers to user account
      await oapApplyMCPServer(selectedServers)
      
      // Trigger force config refresh to sync MCP servers to local (clear cache)
      const refreshResponse = await fetch("/api/plugins/oap-platform/config/force-refresh", { 
        method: "POST" 
      })
      
      if (!refreshResponse.ok) {
        console.error("Config refresh failed:", refreshResponse.status, refreshResponse.statusText)
        showToast({
          message: t("tools.oap.config_refresh_failed") || "Failed to sync MCP servers to local. Please check your authentication.",
          type: "error",
        })
      } else {
        showToast({
          message: t("tools.oap.config_synced") || "MCP servers synced successfully",
          type: "success",
        })
      }
      ...
      console.error("Error applying MCP servers:", error)
      showToast({
        message: t("tools.oap.apply_failed") || "Failed to apply MCP servers",
        type: "error",
      })
    } finally {
  ...
  const handleBannerUrl = (url: string | null | undefined) => {
    if (!url) {
      // Return a default banner image or placeholder
      return `${OAP_ROOT_URL}/images/default-mcp-banner.svg`
    }
