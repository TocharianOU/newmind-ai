...
def define_env(env):
	...
	def templatesWidget(title: str, slug: str, toLoad: int = 3) -> str:
		...
		return '<div class="attacktrace-templates-widget"><p>Template browsing is not available in offline mode.</p></div>'
	...
	def workflowDemo(workflow_json):
		...
		if parsed_workflow_url.scheme == "file":
			try:
				...
				return f"<div class='attacktrace-workflow-preview'><n8n-demo hidecanvaserrors='true' clicktointeract='true' frame='false' collapseformobile='false' workflow='{encoded_workflow_json}'></n8n-demo><a href='{template_url}' target='_blank'>View workflow file</a></div>"
			except (FileNotFoundError, json.JSONDecodeError, KeyError) as e:
				return f"<div class='attacktrace-workflow-preview'><p>Workflow preview unavailable (local file not found).</p></div>"
		else:
			# No external API calls allowed
			return "<div class='attacktrace-workflow-preview'><p>Workflow preview unavailable (external URLs disabled for offline mode).</p></div>"
