...
// Default to true for better debugging experience
const ENABLE_LOGGING =
  process.env.ENABLE_LOGGING !== "false" && process.env.ENABLE_LOGGING !== "0";

// Check if running in stdio mode (MCP_TRANSPORT not set or not 'http')
const IS_STDIO_MODE = !process.env.MCP_TRANSPORT || process.env.MCP_TRANSPORT !== 'http';
...
export function log(type: LogType = "info", ...args: any[]): void {
  ...
  if (IS_STDIO_MODE) {
    // Always use console.error (stderr) in stdio mode
    console.error(`[${type.toUpperCase()}]`, ...args);
    return;
  }

  // In HTTP mode, we can use different console methods
  switch (type) {
    case "info":
      console.info(...args);
      break;
