...
> **Original Author:** [@tocharian](https://github.com/tocharian)  
> **Original Repository:** [https://github.com/tocharian/mcp-server-mysql](https://github.com/tocharian/mcp-server-mysql)  
> **License:** MIT  

## MCP Server for MySQL based on NodeJS

[![Trust Score](https://archestra.ai/mcp-catalog/api/badge/quality/tocharian/mcp-server-mysql)](https://archestra.ai/mcp-catalog/tocharian__mcp-server-mysql)
...
There are several ways to install and configure the MCP server but the most common would be checking this website [https://smithery.ai/server/@tocharian/mcp-server-mysql](https://smithery.ai/server/@tocharian/mcp-server-mysql)

### Cursor

For Cursor IDE, you can install this MCP server with the following command in your project:

1. Visit [https://smithery.ai/server/@tocharian/mcp-server-mysql](https://smithery.ai/server/@tocharian/mcp-server-mysql)
...
# Using pnpm
pnpm add -g @tocharian/mcp-server-mysql
```
...
To run in remote mode, you'll need to provide [environment variables](https://github.com/tocharian/mcp-server-mysql?tab=readme-ov-file#environment-variables) to the script.
...
2. Copy-paste [example file](https://github.com/tocharian/mcp-server-mysql/blob/main/.env) from this repository
...
#### 1. **query_database**
Execute SELECT queries against MySQL database.

**Parameters:**
- `connection_name` (required): Connection identifier
- `query` (required): SQL query string (SELECT/SHOW/DESCRIBE/EXPLAIN)
...
**Parameters:**
- `connection_name` (required): Connection identifier
- `object_name` (optional): Table name (omit to list all tables)
- `token_limit` (optional): Result size limit in tokens (default: 8000)
- `break_token_rule` (optional): Bypass token limit (default: false)

**Returns:**
- Table list (if no object_name specified)
...
```bash
smithery configure @tocharian/mcp-server-mysql
```
...
### Transport Mode Settings (NEW)

- `MCP_TRANSPORT`: Transport mode selection (default: "stdio")
  - `stdio`: For Claude Desktop and local MCP clients (default)
  - `http`: Runs as a standalone HTTP server for remote access and API integration
...
- **HTTP mode**: Runs as a standalone HTTP server with:
  - MCP endpoint at `http://host:port/mcp`
...
- `IS_REMOTE_MCP`: Enable remote MCP mode with authentication (default: "false")
...
**Note:** The new HTTP transport mode (`MCP_TRANSPORT=http`) is recommended over the legacy remote MCP mode for most use cases.
...
7. I am encountering `Error [ERR_MODULE_NOT_FOUND]: Cannot find package 'dotenv' imported from` error
   Make sure all dependencies are properly installed in the mcp-server-mysql directory:
...
[https://github.com/tocharian/mcp-server-mysql](https://github.com/tocharian/mcp-server-mysql)

## Many Thanks to the following Contributors

[![Contributors](https://contrib.rocks/image?repo=tocharian/mcp-server-mysql)](https://github.com/tocharian/mcp-server-mysql/graphs/contributors)
