...
// Create Elasticsearch client factory function (lazy initialization)
...
export async function createElasticsearchMcpServer(
  ...
  // Don't create client here - create it lazily in each tool
  ...
  server.tool(
    ...
    async ({ index }) => {
      try {
        ...
        const response = await client.cat.shards(params);
